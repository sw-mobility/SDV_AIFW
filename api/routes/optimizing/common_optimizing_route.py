...
from models.optimizing.common_optimizing_model import (
   OptimizingHistory
)
...
router = APIRouter(tags=["Optimizing"])

@router.get("/list", status_code=status.HTTP_200_OK)
async def list_optimizing_histories(uid: str = Depends(get_uid)):
    init_result = await init(uid)
    mongo_client = init_result["mongo_client"]

    histories = await mongo_client.db["opt_hst"].find({"uid": uid}).to_list(length=None)
