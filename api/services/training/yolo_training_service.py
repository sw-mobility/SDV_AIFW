...
async def create_training_history(result):
    ...
    tid = result.tid if hasattr(result, 'tid') and result.tid else await get_next_hst_counter(mongo_client, "trn_hst", uid=result.uid, pid=result.pid, prefix="T", field="tid", width=4)
    ...
    processed_dataset_name = f"{origin_dataset_name}({result.did})_preprocessed_{result.origin_tid}"
...
async def upload_preprocessed_dataset(uid: str, origin_did: str, processed_did: str, pid: str, tid: str, task_type: str, name: str, dataset_basepath: str, classes: list):
    ...
    # 원본 데이터셋 정보
    origin_dataset = await mongo_client.db["labeled_datasets"].find_one({"_id": uid+origin_did})
    origin_dataset_name = origin_dataset.get("name", "Unknown Dataset") if origin_dataset else "Unknown Dataset"
    ...
    dataset_info_obj = P_LabeledDatasetInfo(
        ...
        pid = pid,
        name = name,
        description = f"This is a preprocessed dataset of {origin_dataset_name}({origin_did}) for training run {tid[1:]} in project {project_name}.",
        classes = classes,
